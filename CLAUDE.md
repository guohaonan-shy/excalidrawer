...
- `skills/` — one skill per diagram type plus a shared base, modeled on [lark-cli](https://github.com/larksuite/cli/tree/main/skills):
  - `skills/excalidrawer-shared/` — the read-first base. `SKILL.md` holds every cross-cutting rule (MCP precheck + CLI fallback, output naming, label language, export-format selection, render call, iteration); `references/` holds `sugar.md` + `colors.md`. Its description is deliberately non-triggering so it is not invoked as a standalone diagram skill.
  - `skills/excalidrawer-{flowchart,timeline,architecture,sequence}/` — each declares a **前置条件: read `../excalidrawer-shared/SKILL.md` first** (live sibling, lark-style), then adds only its own clarify questions + `references/<type>.md` recipe. Recipes reference the shared schema at `../../excalidrawer-shared/references/sugar.md`.
  - **Why live sibling, not self-contained copies:** a whole-repo / plugin install lays all skill folders flat as siblings under `.agents/skills/`, so `../excalidrawer-shared/` resolves. The only degraded path is `npx skills add … --skill <one>`, which pulls a single folder and leaves the sibling ref dangling — so the README steers users to install everything (plugin or whole-repo) and documents installing `excalidrawer-shared` alongside any single skill. This keeps shared docs single-source (no 4× duplication, no sync script).
  - Skills do NOT configure MCP; `excalidrawer-shared/SKILL.md` §1 documents the `claude mcp add excalidrawer -- excalidrawer-mcp` prerequisite.
