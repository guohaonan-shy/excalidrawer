---
name: excalidrawer
description: Generate hand-drawn-style Excalidraw diagrams from code and export to .excalidraw / SVG / PNG — no browser. Covers four diagram types — flowchart (decision flows, branching, approval pipelines), timeline (roadmaps, milestones, project phases, Q1–Q4), architecture (system layers, 3-tier, microservices, service topology, data platform), and sequence diagrams (multi-actor interactions, API call order, handshakes, OAuth flows) — plus freeform topology (trees, mind maps, org charts). Triggers on any request to create a diagram / draw a flowchart / visualize a process / export to svg or png. Flow: detect type → AskUserQuestion clarify → read the matching reference recipe → compose sugar elements → call the excalidrawer MCP server's render_diagram. 中文触发词：流程图 / 决策流 / 审批流 / 时间线 / 路线图 / 里程碑 / 架构图 / 分层 / 微服务 / 服务拓扑 / 时序图 / 多角色交互 / 画个图 / 导出 svg png。
